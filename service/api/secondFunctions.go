...
func ( rt *_router) getMyConversations(w http. ResponseWriter, r *http.Request, ps httprouter.Params){
	...
	for i:=0; i<len(conversationIds);i++{
		...
		if err!=nil{fmt.Println("error in function GetConversation")}
		if toAdd.Groupchat==false{
			var members []string
			members, err = rt.db.GetAllMembers(conversationIds[i])
			userIdstr := strconv.Itoa(userId)
			if members[0]== userIdstr{
				var receiverIdint int
				receiverIdint,err = strconv.Atoi(members[1])
				var receiver database.User
				receiver, err = rt.db.GetUser(database.UserId{receiverIdint})
	...
	if err!= nil{fmt.Println("error in GetConversations function")}
...
func ( rt *_router) startNewConversation(w http. ResponseWriter, r *http.Request, ps httprouter.Params){
	...
	if len(receivers)==1{
		...
		if err != nil{fmt.Println("error in function CheckIfInConversation")}
